...
LK_motor::LK_motor(CAN_HandleTypeDef *hcan_, can_rx_callback *callback_, uint8_t is_use_multi_ctrl_, uint32_t id_motor, LK_motor_type motor_type, uint8_t is_reverse, uint8_t is_use_motor_pid_)
         : can_rx(hcan_,0x140+id_motor,callback_), can_tx(hcan_,is_use_multi_ctrl_?0x280:0x140+id_motor), default_data_rx{},
           is_pid_send_success(false), is_init_success(true), mode(torque_ctrl), type(motor_type), is_use_multi_ctrl(is_use_multi_ctrl_),
...
void LK_motor::motor_set_rounds_forward(float rounds) {
    switch(mode){
        ...
        case torque_ctrl:
            {
                float current_rounds = motor_get_rounds_forward();
                float diff = rounds - current_rounds;
                // 最短路径逻辑：将差值限制在 [-0.5, 0.5] 圈内
                while (diff > 0.5f) diff -= 1.0f;
                while (diff < -0.5f) diff += 1.0f;
                
                float shortest_target = current_rounds + diff;
                float set = posPid.pid_calculate(shortest_target, current_rounds);
...
uint32_t LK_motor::motor_get_ecd(void) const {
    return (uint32_t)(motor_data.ecd);
}

float LK_motor::motor_get_angle_deg(void) const {
    int32_t max_ecd = (int32_t)(motor_param.max_ecd);
    int32_t half_ecd = max_ecd / 2;
    int32_t delta_ecd = (int32_t)(motor_data.offset_ecd - motor_data.ecd);

    if (delta_ecd > half_ecd) {
        delta_ecd -= max_ecd;
    } else if (delta_ecd < -half_ecd) {
        delta_ecd += max_ecd;
    }

    return (float)(delta_ecd) * 360.0f / motor_param.max_ecd;
}
