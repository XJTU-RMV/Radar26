...
import os
import threading
from driver.referee.protocol import Crc, MsgID
from driver.referee.messages import RefereeGenericMessage
...
class RefereeSerialManager():
    def __init__(self, port=None, baudrate=115200, auto_scan=True, excluded_ports=None):
        self.initial_port = port
        self.baudrate = baudrate
        self.auto_scan = auto_scan
        self.excluded_ports = set(excluded_ports or []) # 排除云台占用的串口
        ...
        # Logging - 使用相对路径，避免权限问题
        self.recording_save_root_dir = os.path.join(os.getcwd(), "logs", "referee_logs")
    ...
    def _find_available_port(self):
        ...
        if self.initial_port and os.path.exists(self.initial_port):
            if self.initial_port in self.excluded_ports:
                print(f"Specified port {self.initial_port} is excluded")
                return None
        ...
        if self.auto_scan:
            ...
            for port_info in usb_ports:
                ...
                if port in self.excluded_ports:
                    print(f"Skipping excluded port: {port}")
                    continue
...
def main(port, baudrate):
    ...
    try:
        while True:
            ...
            from .messages import Radar2SentryMessage, Radar2ClientMessage
            radar2client_msg = Radar2ClientMessage(
                opponent_hero_x=100,
                opponent_hero_y=200,
                opponent_engineer_x=300,
                opponent_engineer_y=400,
                opponent_standard_3_x=500,
                opponent_standard_3_y=600,
                opponent_standard_4_x=700,
                opponent_standard_4_y=800,
                opponent_aircraft_x=900,
                opponent_aircraft_y=1000,
                opponent_sentry_x=1100,
                opponent_sentry_y=1200,
                ally_hero_x=1300,
                ally_hero_y=1400,
                ally_engineer_x=1500,
                ally_engineer_y=1600,
                ally_standard_3_x=1700,
                ally_standard_3_y=1800,
                ally_standard_4_x=1900,
                ally_standard_4_y=2000,
                ally_aircraft_x=2100,
                ally_aircraft_y=2200,
                ally_sentry_x=2300,
                ally_sentry_y=2400,
            )
