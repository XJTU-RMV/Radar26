from model.mobilenet.model import (
    MobileNetClassifier,
...
from model.mobilenet.transform import NormalTransform
import torch
    ...
    def __init__(self, model_type, weights_path, class_names=["1", "2", "3", "4", "S", "Q"]):
        ...
        if not os.path.exists(weights_path):
            raise FileNotFoundError(
                f"Digit classifier weights file not found: {weights_path}\n"
                f"Please download the model weights and place them in the weights/ directory."
            )
