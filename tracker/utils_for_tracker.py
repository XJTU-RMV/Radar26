...
def compute_iou(bbox1: List[Union[float, int]], bbox2: List[Union[float, int]]):
    """
...
def xyxy2xywh(bbox: List[Union[float, int]]) -> List[float]:
    """
...
def xywh2xyxy(bbox: List[Union[float, int]]) -> List[float]:
    """
...
def compute_normalized_center_distance(
    bbox1: List[Union[float, int]], bbox2: List[Union[float, int]], img: np.ndarray
) -> float:
    """
...
def compute_center_distance_xy(bbox1: List[Union[float, int]], bbox2: List[Union[float, int]]):
    center1 = [(bbox1[0] + bbox1[2]) / 2, (bbox1[1] + bbox1[3]) / 2]
    center2 = [(bbox2[0] + bbox2[2]) / 2, (bbox2[1] + bbox2[3]) / 2]
